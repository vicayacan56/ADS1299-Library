...
`test_ads1299_core.cpp` compiles the portable core without Arduino stubs. `test_ads1299_protocol.cpp` compiles the unintegrated protocol object against a local HAL fake. `test_ads1299_host.cpp` uses minimal local stubs for `Arduino.h` and `SPI.h`, plus a `FakeHAL` that records SPI/GPIO/timing calls and returns queued SPI bytes.

## What is covered

- Portable `ADS1299Core` helpers
- Unintegrated `ADS1299_Protocol` skeleton
